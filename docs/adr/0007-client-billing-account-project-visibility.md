...
## Addendum — project recency fields (2026-09-23)

`fetch-client-projects`'s `PROJECT_ROW_SHAPE` now also surfaces `lastActivityAt` and `lastActivityUserId` from each `GET /v6/projects?billingAccountId=` row, both optional and normalized to strings via the existing `toStringOrUndefined()` (the upstream user id may arrive as a number). No extra upstream call is involved — the fields were already present in the list response and were simply being dropped by the row mapper. Rationale: the tool's primary use ("find me all the work done for client X") naturally leads to "which of these are still active / most recent?", which previously required a per-project `fetch-project-by-id` follow-up. `challengeSearchAgent`'s "Finding a client's work across projects" instructions were extended to use `lastActivityAt` for recency questions and ordering, and to treat absence as "not recorded" rather than an error. `fetchProjectTool` (`fetch-project-by-id`) surfaces the same two fields on its `PROJECT_SHAPE` — mapped in the shared `mapProject()`, so they appear on both the primary `project` and every name-search `matches` entry — keeping a project's recency consistent whichever of the two tools the agent reached it through. Again no new upstream call: both `GET /v6/projects/:projectId` and `GET /v6/projects?name=` already return them (a caller-supplied `fields` narrowing can omit them, in which case they're simply absent). The tool description and the agent's project-detail guidance ("Keep projects separate") mention them for "when was this project last active" questions. `lastActivityUserId` is a raw Topcoder user id, not a handle — it is not resolved further here. (applies to both tools).

## Addendum — project members (2026-09-24)

`fetchProjectTool` (`fetch-project-by-id`) now surfaces the project team as an optional `members` array on `PROJECT_SHAPE`, sourced from the `members` key `GET /v6/projects/:projectId` already returns — no extra upstream call, same requestor-token-only credential posture as the rest of the tool (Decision 2). Each upstream row looks like:

```json
{ "id": "1000335", "projectId": "1000289", "userId": "8547899", "role": "manager", "isPrimary": true,
  "deletedAt": null, "createdAt": "2025-01-07T18:23:17.853Z", "updatedAt": "...", "deletedBy": null,
  "createdBy": 8547899, "updatedBy": 8547899, "handle": "TonyJ" }
```

It is mapped (in the shared `mapProject()`, via `mapMembers()`) to `{ userId, handle?, role?, isPrimary?, createdAt? }`: `userId` is coerced to string like every other id this tool returns, rows with `deletedAt` set or no `userId` are dropped, and the audit fields (`projectId`, `updatedAt`, `createdBy`/`updatedBy`/`deletedBy`, the membership row `id`) are not surfaced. Absent `members` upstream (e.g. a `fields` narrowing) → absent in output, not an empty array. On the name-search path, `members` are kept on the best-match `project` only and stripped from `matches` — those exist for disambiguation, and a roster per alternative would bloat the tool output for no benefit.

Rationale: the business need this ADR serves ("visibility to work delivered for a specific client") naturally reaches "who is delivering it" — the project manager, copilot, and customer contact — which was previously invisible to the agent: `fetch-challenge-resources` (ADR 0005) is per-challenge, not per-project. `challengeSearchAgent`'s "Keep projects separate" guidance was extended to explain what the project roles mean for delivery (manager/copilot/staff roles vs. `customer` vs. `observer`), to treat `isPrimary` as the main contact for a role, to group members by role with linked handles, to name the member behind `lastActivityUserId` when it matches a member's `userId`, to offer `fetch-member-insights` on a member without calling it unprompted, and to not treat project membership as evidence of who worked on a given challenge (that remains `fetch-challenge-resources`'s job). Missing `members` is to be reported as "team not visible", not "no members". The tool description and the README's `fetch-project-by-id` section mention the field.
