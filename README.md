...
| `fetch-project-by-id` | Resolve a `projectId` reference to project detail and team members, plus its client/subcontracting end customer | `challengeSearchAgent` (on-demand enrichment) — **restricted**: `administrator`/`Talent Manager` only (ADR 0007) |
...
| **Output** | `{ project: { id, name?, status?, type?, billingAccountId?, directProjectId?, techStack?, lastActivityAt?, lastActivityUserId?, members?: [{ userId, handle?, role?, isPrimary?, createdAt? }], client?: { id, name?, codeName? }, subcontractingEndCustomer? } }` |
| **Access** | `restricted` — `roles: ['administrator', 'Talent Manager']`, no `scopes` (all M2M callers denied) — was `public` before ADR 0007 |

Retrieval-time enrichment only (not used by ingestion): resolves the opaque `projectId` a challenge-search hit carries into project name/status/tech stack/last activity (`lastActivityAt`, `lastActivityUserId` — both optional, passed through as strings, same as `fetch-client-projects`' project rows) and project team `members` (`userId`, `handle`, project `role` such as `manager`/`copilot`/`customer`, `isPrimary`, `createdAt` — passed through from the project detail response when present; soft-deleted rows and audit fields are dropped, and name-search `matches` never carry them), under the caller's own authorization. The agent uses `members` for "who manages/copilots/is the customer on this project" questions and to put a handle on `lastActivityUserId` — distinct from `fetch-challenge-resources`, which lists who is on a single challenge. Since [ADR 0007](docs/adr/0007-client-billing-account-project-visibility.md), also enriches the result with the project's billing-account `client` and `subcontractingEndCustomer` (a plain string, not an object — confirmed against a real response) when the project has a `billingAccountId`. This enrichment is **fail-soft**: a missing `billingAccountId`, a non-2xx billing-account response, or a thrown error is logged and swallowed, returning the base project data without `client`/`subcontractingEndCustomer` rather than failing the whole call — and it **never** escalates to tc-ai-api's own M2M token, for any caller, so a `Talent Manager` whose own JWT can't see a given billing account silently gets the project back without client data rather than the tool escalating privilege on their behalf.
